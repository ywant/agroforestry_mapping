...
class BinaryTverskyLoss(torch.nn.Module):
    def __init__(self, alpha=0.4, beta=0.6, weight=None, size_average=True):
    ...
    def forward(self, inputs, targets, weights=None):
        ...
        if weights is not None:
            TP = (weights * inputs * targets).sum([1,2,3])
            FP = (weights * (1 - targets) * inputs).sum([1,2,3])
            FN = (weights * targets * (1 - inputs)).sum([1,2,3])
        else:
            TP = (inputs * targets).sum([1,2,3])
            FP = ((1 - targets) * inputs).sum([1,2,3])
            FN = (targets * (1 - inputs)).sum([1,2,3])

        Tversky = (TP) / (TP + self.alpha * FP + self.beta * FN + epsilon)
